...
You are welcome to leave `Tab Size` to its default value in VS Code (4 spaces) or update it to increase code visibility as desired. For a reference on how to update `Tab Size` please see [lesson on VsCode](/pre-work/getting-started-with-intro-to-programming/text-editor-visual-studio-code) from the pre-work. 
